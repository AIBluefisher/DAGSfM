...
DistributedMapperController::DistributedMapperController(
    const Options& options, const ImageClustering::Options& clustering_options,
...
void DistributedMapperController::Run() {
  ...
  if (options_.retriangulate) {
    ...
    triangulator.reset(new IncrementalTriangulator(
        global_recon.GetCorrespondenceGraph(), &global_recon));
  ...
  if (options_.final_ba && (image_clustering_->GetInterClusters().size() > 1)) {
    ...
    timer.Pause();
    LOG(INFO) << "Final bundle adjustment took: " << timer.ElapsedSeconds()
              << " seconds.";
  }
...
bool DistributedMapperController::RunSequential() {
  ...
  ReconstructPartitions(image_id_to_name_, inter_clusters_,
                        reconstruction_managers, reconstructions);
  ...
  MergeClusters(inter_clusters_, reconstructions, reconstruction_managers,
                num_eff_threads);
...
bool DistributedMapperController::RunDistributed() {
  ...
  for (size_t k = 0; k < inter_clusters_.size(); k++) {
    ...
    for (const auto image_id : inter_clusters_[k].image_ids) {
      image_name_list.push_back(image_id_to_name_.at(image_id));
    }
  ...
  //// 3. Merge clusters ///////////////////////////
  //////////////////////////////////////////////////
  std::vector<Reconstruction*>
      reconstructions;  //= sfm_data_container.reconstructions;
...
void DistributedMapperController::RepartitionScenesForMVS() {
  ...
  for (uint i = 0; i < reg_image_centers.size(); i++) {
    for (uint j = i + 1; j < reg_image_centers.size(); j++) {
      ...
      const ViewIdPair view_pair =
          reg_image_ids[i] > reg_image_ids[j]
  ...
  for (uint i = 0; i < intra_clusters.size(); i++) {
    const std::string reconstruction_path =
        JoinPaths(options_.output_path, "tpartition_" + std::to_string(i));
    ...
    WritePoints3DBinary(points3D,
                        JoinPaths(reconstruction_path, "points3D.bin"));
  }
}
...
void DistributedMapperController::ExportUntransformedLocalRecons(
  ...
  for (size_t i = 0; i < reconstructions.size(); ++i) {
    const std::string reconstruction_path =
        JoinPaths(options_.output_path, "partition_" + std::to_string(i));
    CreateDirIfNotExists(reconstruction_path);
