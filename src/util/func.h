...
bool comPair(std::pair<weightPair, colmap::image_pair_t> A, std::pair<weightPair, colmap::image_pair_t> B)
{
...
bool compView(std::pair<int, Eigen::Vector2d> A, std::pair<int, Eigen::Vector2d> B)
{
...
bool comAngle(std::pair<weightPair, colmap::image_pair_t> A, std::pair<weightPair, colmap::image_pair_t> B)
{
    if(A.first.weight > B.first.weight)
        return true;
...
bool comInlier(std::pair<weightPair, colmap::image_pair_t> A, std::pair<weightPair, colmap::image_pair_t> B)
{
    if(A.first.nInliers > B.first.nInliers)
        return true;
...
int mergeTwoClusters(std::map<colmap::image_t, size_t>& imgCluster, colmap::image_t id1, colmap::image_t id2)
{
...
bool comEdges(std::pair<colmap::image_pair_t, double> A, std::pair<colmap::image_pair_t, double> B)
{
    if(A.second > B.second)
        return true;
    else
...
colmap::image_pair_t ComputePairID(colmap::image_t image_id1, colmap::image_t image_id2)
{
...
double ComputeRotationError(const Eigen::Matrix3d& R12, const Eigen::Matrix3d& R12_5points)
{
...
double ComputeRaysAngle(const Eigen::Vector3d& ray1, const Eigen::Vector3d& ray2)
{
...
void Transform2LocalTracks(tracks::STLMAPTracks& map_tracks, vector<track>& tracks_,
                           ...
                           std::map<colmap::image_pair_t, FeatureMatches>& ImgMatches)
{
    ...
    for(auto& trackMap : map_tracks)
    {
        ...
        if(sTrack.size() < static_cast<size_t>(2) || sTrack.size() >= static_cast<size_t>(100))
            continue;
        ...
        {
            std::map<int, int>& visibleTracks = images_[subTrack.first].VisibleTracks();
            curTrack.view2D[subTrack.first] = subTrack.second;
            if(visibleTracks.count(subTrack.second) > 0) /// one feature exist in many tracks
            {
                flag = true;
                break;
            }
    ...
    for(auto& trackV : trackViewData)
    {
        if(trackV.first < 10)
        {
            if(trackV.first == 0)
            {
                LOG(INFO) << 3 << " - " << (trackV.first + 1) * 10 << " : " << trackV.second << std::endl;
            }
            else
            {
                LOG(INFO) << trackV.first * 10 << " - " << (trackV.first + 1) * 10 << " : " << trackV.second << std::endl;
            }
        }
        else
            LOG(INFO) << trackV.first * 10 << " - infinite" << " : " << trackV.second << std::endl;
    }
}
}
