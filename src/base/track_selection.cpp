    ...
    const int long_track_length_threshold)
{
    ...
    // Compute the squared reprojection error for each view that observes this track
    // and add it to the accumulating sum.
    for (const TrackElement element : track_elements) {
        if (!reconstruction.IsImageRegistered(element.image_id)) { continue; }
        ...
        sq_reprojection_error_sum += 
            colmap::CalculateSquaredReprojectionError(point2d.XY(), point3d.XYZ(), 
    ...
    std::unordered_map<point3D_t, TrackStatistics>* track_statistics)
{
    ...
    for (const image_t image_id : image_ids) {
        ...
        for (const Point2D& point2d : points2d) {
            ...
            if (track_statistics->count(track_id) != 0) { continue; }

            // Compute the track statistics and add it to the output map.
            const TrackStatistics& statistics_for_this_track =
                ComputeStatisticsForTrack(reconstruction, track_id, long_track_length_threshold);
...
// do this, we first bin the tracks into grid cells in an image grid. Then within
// each cell we find the best ranked track and add it to the list of tracks to optimize.
    ...
    std::unordered_set<point3D_t>* tracks_to_optimize)
{
    ...
    for (const Point2D& point2d : points2d) {
        ...
        const Eigen::Vector2i grid_cell = (feature * inv_grid_cell_size).cast<int>();

        image_grid[grid_cell].emplace_back(track_id, current_track_statistics);
    }

    // Select the best feature from each grid cell and add it to the tracks to optimize.
        ...
        if (tracks_to_optimize->count(track_id) != 0) {
            ...
            if (track_statistics.count(track_id) != 0) {
                ranked_candidate_tracks.emplace_back(
                    track_id, track_statistics.at(track_id));
            }
        }
    }
    ...
    if (num_optimized_tracks != num_estimated_tracks) {
        ...
        const int num_optimized_tracks_needed = 
            std::min(min_num_optimized_tracks_per_view, num_estimated_tracks) - 
                num_optimized_tracks;
        std::partial_sort(ranked_candidate_tracks.begin(),
...
//    c) The tracks used for optimization provide a good spatial coverage in each image.
