 ...
 * Adapted from the i23dSFM libraries,
 ...
 * - We do not save the original, instead we rely on the undistorted image from i23dSFM.
 * - We do not output thumbnails or EXIF blobs, as these appear only to be used only for the GUI UMVE.
 * - To avoid encoding loss, i23dSFM images should be written as .PNG if undistorted images are *not* computed.
 * - In I23dSFM, some views may have some missing poses; MVE does *not* require a contiguous camera index.
 *
  ...
  )
{
  ...
  // Export the SfM_Data scene to the MVE2 format
  {
    ...
    // Get cameras and features from I23dSFM
      ...
      iter != sfm_data.GetViews().end(); ++iter, ++my_progress_bar)
    {
      ...
      if (sfm_data.IsPoseAndIntrinsicDefined(view))
      {
        ...
        // To do:  trim any extra separator(s) from i23dSFM name we receive, e.g.:
        // '/home/insight/i23dSFM_KevinCain/i23dSFM_Build/software/SfM/ImageDataset_SceauxCastle/images//100_7100.JPG'
        std::ofstream file(
