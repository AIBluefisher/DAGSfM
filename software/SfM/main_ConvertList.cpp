...
// Convert an i23dSFM v0.x=>7 lists.txt file to the new SfM_Data file format (v0.8)
...
int main(int argc, char **argv)
{
  ...
  std::vector<i23dSFM::SfMIO::CameraInfo> vec_camImageNames;
  std::vector<i23dSFM::SfMIO::IntrinsicCameraInfo> vec_intrinsicGroups;
  if (!i23dSFM::SfMIO::loadImageList( vec_camImageNames,
                                      vec_intrinsicGroups,
                                      sListsFile) )
  {
  ...
  for (std::vector<i23dSFM::SfMIO::CameraInfo>::const_iterator iter = vec_camImageNames.begin();
    iter != vec_camImageNames.end(); ++iter)
  {
    const i23dSFM::SfMIO::CameraInfo & camInfo = *iter;
    // Find the index of the correponding cameraInfo
    const size_t idx = std::distance((std::vector<i23dSFM::SfMIO::CameraInfo>::const_iterator)vec_camImageNames.begin(), iter);

    double width = height = ppx = ppy = focal = -1.0;

    std::shared_ptr<IntrinsicBase> intrinsic (NULL);

    const i23dSFM::SfMIO::IntrinsicCameraInfo & camIntrinsic = vec_intrinsicGroups[camInfo.m_intrinsicId];
