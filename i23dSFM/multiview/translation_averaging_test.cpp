...
#include "i23dSFM/multiview/translation_averaging_test.hpp"
...
TEST(translation_averaging, globalTi_from_tijs_Triplets_softL1_Ceres) {
  ...
  std::vector<i23dSFM::relativeInfo > vec_relative_estimates;
...
TEST(translation_averaging, globalTi_from_tijs_Triplets_l2_chordal) {
  ...
  {
    const i23dSFM::relativeInfo & rel = vec_relative_estimates[i];
