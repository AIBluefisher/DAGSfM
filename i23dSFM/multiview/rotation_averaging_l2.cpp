...
#include "i23dSFM/multiview/rotation_averaging_l2.hpp"
#include <vector>
#include <map>
...
struct CeresPairRotationError {
  CeresPairRotationError(const i23dSFM::Vec3& relative_rotation,  const double weight)
  ...
  const i23dSFM::Vec3 relative_rotation_;
  const double weight_;
};

bool L2RotationAveraging_Refine(
  const RelativeRotations & vec_relativeRot,
  std::vector<i23dSFM::Mat3> & vec_ApprRotMatrix)
{
  ...
  std::vector<i23dSFM::Vec3> vec_Rot_AngleAxis(vec_ApprRotMatrix.size());
  ...
  for (size_t ii = 0; ii < vec_relativeRot.size(); ++ii)
  {
    ...
    const i23dSFM::Mat3& Rrel = vec_relativeRot[ii].Rij;
    const double w = vec_relativeRot[ii].weight;

    i23dSFM::Vec3 rotAngleAxis;
