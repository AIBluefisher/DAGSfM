...
{
  i23dSFM::image::Image< double > map;

  IntegralImages(const i23dSFM::image::Image< float >& I);
...
std::ofstream& writeDetector( std::ofstream& out, const i23dSFM::features::SIOPointFeature& vect );
std::ifstream& readDetector( std::ifstream& in, i23dSFM::features::SIOPointFeature& point );
...
float getRange(const i23dSFM::image::Image< float >& I, int a, const float p);
