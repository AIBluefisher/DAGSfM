...
struct ImageScale
{
	std::vector< i23dSFM::image::Image< float > > angles;
  std::vector< i23dSFM::image::Image< float > > magnitudes;
	std::vector< double > ratios;
	double radius_size;
	double step;

  ImageScale(const i23dSFM::image::Image< float >& I, double r = 5.0);
	int getIndex( const double r )const;

private:
  void GradAndNorm(
    const i23dSFM::image::Image< float >& I,
    i23dSFM::image::Image< float >& angle,
    i23dSFM::image::Image< float >& m);
};
...
float KVLD(const i23dSFM::image::Image< float >& I1,
  const i23dSFM::image::Image< float >& I2,
  const std::vector<i23dSFM::features::SIOPointFeature> & F1,
  const std::vector<i23dSFM::features::SIOPointFeature> & F2,
  const std::vector< i23dSFM::Pair >& matches,
  std::vector< i23dSFM::Pair >& matchesFiltered,
