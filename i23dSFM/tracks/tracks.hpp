...
#ifndef I23DSFM_TRACKS_H_
#define I23DSFM_TRACKS_H_
...
struct TracksBuilder
{
  ...
  bool ExportToFile(const std::string& filename, const STLMAPTracks& map_tracks)
  {
    std::ofstream stream(filename);
    if (!stream.is_open()) {
      std::cout << "cannot create tracks file " << filename << std::endl;
      return false;
    }
    for (auto mti = map_tracks.begin(); mti != map_tracks.end(); mti++) {
      size_t track_id = mti->first;
      const submapTrack& sub_track = mti->second;
      stream << track_id << " " << sub_track.size() << "\n";
      for (auto ti = sub_track.begin(); ti != sub_track.end(); ti++) {
        stream << ti->first << " " << ti->second << "\n";
      }
    }
    stream.close();
    return true;
  ...
  bool LoadFromFile(const std::string& filename, STLMAPTracks& map_tracks) 
  {
    std::ifstream stream(filename);
    if (!stream.is_open()) {
      std::cout << "cannot open tracks file " << filename << std::endl;
      return false;
    }

    size_t track_id;
    size_t size;
    while (stream >> track_id >> size) {
      submapTrack sub_track;
      for (int i = 0; i < size; i++) {
        size_t image_id, feature_id;
        stream >> image_id >> feature_id;
        sub_track.insert(make_pair(image_id, feature_id));
      }
      map_tracks.insert(make_pair(track_id, sub_track));
    }
    return map_tracks.size() > 0;
  }
