...
#ifndef I23DSFM_SFM_DATA_IO_PLY_HPP
#define I23DSFM_SFM_DATA_IO_PLY_HPP
  ...
  ESfM_Data flags_part)
{
  ...
  bool bOk = false;
  {
    ...
    stream << "ply"
      ...
      //const Vec3 sensor[4] = {Vec3(0.01, 0.01, 0.01), Vec3(-0.01, 0.01, 0.01), Vec3(-0.01, -0.01, 0.01), Vec3(0.01, -0.01, 0.01)};
	  ...
	  if (b_extrinsics)
      {
        for (const auto & view : sfm_data.GetViews())
        {
          if (sfm_data.IsPoseAndIntrinsicDefined(view.second.get()))
          {
			...
			stream << (pose.rotation().transpose() * Vec3(0, 0, 0.01) + pose.center()).transpose()
              << " 255 0 0" << "\n";
          }
        }
      }
...
#endif // I23DSFM_SFM_DATA_IO_PLY_HPP
