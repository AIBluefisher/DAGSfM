...
#include "i23dSFM/sfm/pipelines/global/mutexSet.hpp"
#include "i23dSFM/multiview/translation_averaging_common.hpp"
#include "i23dSFM/multiview/translation_averaging_solver.hpp"
#include "i23dSFM/graph/graph.hpp"
#include "i23dSFM/stl/stl.hpp"
#include "i23dSFM/system/timer.hpp"
#include "i23dSFM/linearProgramming/linearProgramming.hpp"
#include "i23dSFM/multiview/essential.hpp"
#include "i23dSFM/multiview/conditioning.hpp"
#include "i23dSFM/multiview/translation_averaging_common.hpp"
#include "i23dSFM/multiview/translation_averaging_solver.hpp"
#include "i23dSFM/sfm/pipelines/global/triplet_t_ACRansac_kernelAdaptator.hpp"
  ...
  const Hash_Map<IndexT, Mat3> & map_globalR)
{
  ...
  const std::set<IndexT> set_remainingIds =
    i23dSFM::graph::CleanGraph_KeepLargestBiEdge_Nodes<Pair_Set, IndexT>(pairs, "./");
  ...
  const std::string _sOutDirectory("./");
  {
    ...
    {
      i23dSFM::relativeInfo & rel = vec_initialRijTijEstimates_cpy[i];
      rel.first = Pair(_reindexForward[rel.first.first], _reindexForward[rel.first.second]);
    }

    i23dSFM::system::Timer timerLP_translation;
    ...
    switch(eTranslationAveragingMethod)
    {
      ...
      case TRANSLATION_AVERAGING_L2_DISTANCE_CHORDAL:
      {
        ...
        {
          const i23dSFM::relativeInfo & rel = vec_initialRijTijEstimates_cpy[i];
...
{
  i23dSFM::system::Timer timerLP_triplet;
...

  {
    ...
    for (int i = 0; i < (int)vec_triplets.size(); ++i)
    {
      ...
      // Compute tracks:
      {
        i23dSFM::tracks::TracksBuilder tracksBuilder;
    ...
    i23dSFM::sfm::MutexSet<myEdge> m_mutexSet;
    ...
    for (int k = 0; k < vec_edges.size(); ++k)
    {
      ...
      for (size_t i = 0; i < vec_possibleTriplets.size(); ++i)
      {
        ...
        i23dSFM::tracks::STLMAPTracks pose_triplet_tracks;
...
bool GlobalSfM_Translation_AveragingSolver::Estimate_T_triplet(
  ...
  std::vector<size_t> & vec_inliers,
  i23dSFM::tracks::STLMAPTracks & tracks,
  const std::string & sOutDirectory) const
{
  ...
  using namespace i23dSFM::trifocal;
  using namespace i23dSFM::trifocal::kernel;
