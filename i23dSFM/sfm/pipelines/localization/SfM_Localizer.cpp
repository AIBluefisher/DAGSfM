...
#include "i23dSFM/multiview/solver_resection_kernel.hpp"
#include "i23dSFM/multiview/solver_resection_p3p.hpp"
...
namespace sfm {
  ...
  )
  {
    ...
    if (pinhole_cam == nullptr)
    {
      ...
      typedef i23dSFM::resection::kernel::SixPointResectionSolver SolverType;
      MINIMUM_SAMPLES = SolverType::MINIMUM_SAMPLES;

      typedef i23dSFM::robust::ACKernelAdaptorResection<
        SolverType, ResectionSquaredResidualError, i23dSFM::robust::UnnormalizerResection, Mat34>
        KernelType;
      ...
      const std::pair<double,double> ACRansacOut =
        i23dSFM::robust::ACRANSAC(kernel, resection_data.vec_inliers, resection_data.max_iteration, &P, dPrecision, true);
    ...
    else
    {
      ...
      typedef i23dSFM::euclidean_resection::P3PSolver SolverType;
      MINIMUM_SAMPLES = SolverType::MINIMUM_SAMPLES;

      typedef i23dSFM::robust::ACKernelAdaptorResection_K<
        SolverType, ResectionSquaredResidualError,
        i23dSFM::robust::UnnormalizerResection, Mat34>  KernelType;
